...
const Pruebas = () => {
  ...
  //  SELECCIÓN DE MODELO
  const [selectedModel, setSelectedModel] = useState<'model_1' | 'model_2'>('model_1')
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  //  ANALIZAR IMAGEN - DUAL MODEL SUPPORT
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      console.log(`📤 Enviando imagen con ${selectedModel.toUpperCase()} (multipart)...`)
      ...
      console.log('✅ Análisis completado con', data.procesamiento?.model)
    } catch (err) {
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 bg-cyan-500/10 border border-cyan-500/30 rounded-full px-5 py-2 mb-6">
              ...
              <span className="text-cyan-400 text-sm font-semibold tracking-wide">FRONTEND v6.0 + BACKEND v6.0 DUAL MODEL 🚀</span>
            </div>
            ...
            <p className="text-xl text-slate-400 max-w-3xl mx-auto">
              2 Modelos IA • UNet++ EfficientNet-B8 + ConvNeXt-Base CBAM • TTA • Raspberry Pi • Multipart
            </p>
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-4">
              ...
              <div className="inline-flex items-center gap-2 bg-slate-800/50 border border-slate-700 rounded-full px-6 py-3">
                ...
                <select
                  value={selectedModel}
                  onChange={(e) => setSelectedModel(e.target.value as 'model_1' | 'model_2')}
                  className="bg-slate-700 text-white px-4 py-2 rounded-lg font-semibold cursor-pointer hover:bg-slate-600 transition-all"
                >
                  <option value="model_1">Modelo 1 (EfficientNet-B8 + TTA)</option>
                  <option value="model_2">Modelo 2 (ConvNeXt-Base + CBAM)</option>
                </select>
              </div>

              {/* TTA TOGGLE (solo para Modelo 1) */}
                ...
                <div className="flex items-center justify-center gap-3">
                  ...
                  <div className="text-left">
                    <p className="text-sm font-semibold text-cyan-400">
                      {selectedModel === 'model_1' ? 'Modelo 1:  UNet++ + EfficientNet-B8' :  'Modelo 2: UNet++ + ConvNeXt-Base + CBAM'}
                    </p>
                    <p className="text-xs text-slate-400">
                      {selectedModel === 'model_1' 
                        ? `TTA: ${useTTA ? '✅ Activado (6 transformaciones)' : '❌ Desactivado'} • best_model.pth`
                        : '✅ CBAM Attention • Sin TTA (máxima velocidad) • modelof.pth'
                      }
                    </p>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="mt-8 bg-cyan-500/10 border border-cyan-500/30 rounded-2xl p-6">
                      ...
                      <ul className="text-sm text-slate-300 space-y-3">
                        ...
                        <li className="flex items-start gap-2">
                          <div className="w-1.5 h-1.5 bg-cyan-400 rounded-full mt-2 flex-shrink-0"></div>
                          <span>🆕 Elige entre 2 modelos IA (Modelo 1 con TTA o Modelo 2 con CBAM)</span>
                        </li>
                        <li className="flex items-start gap-2">
                          <div className="w-1.5 h-1.5 bg-cyan-400 rounded-full mt-2 flex-shrink-0"></div>
                        ...
                        <li className="flex items-start gap-2">
                          <div className="w-1.5 h-1.5 bg-cyan-400 rounded-full mt-2 flex-shrink-0"></div>
                          <span>🚀 Backend v6.0: Dual Model + Multipart (3× más rápido)</span>
                        </li>
                        <li className="flex items-start gap-2">
                          <div className="w-1.5 h-1.5 bg-cyan-400 rounded-full mt-2 flex-shrink-0"></div>
                          <span>Recibe diagnóstico con análisis morfológico y severidad</span>
                        </li>
                      </ul>
                    </div>
                  </div>
                      ...
                      {! isProcessing && ! result && (
                        ...
                        >
                          ...
                          <div className="relative bg-gradient-to-r from-green-500 to-emerald-600 text-white py-4 px-6 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 shadow-lg shadow-green-500/50 hover:scale-105">
                            ...
                            Analizar con {selectedModel === 'model_1' ? 'Modelo 1' : 'Modelo 2'}
                            {selectedModel === 'model_1' && useTTA && ' + TTA'}
                          </div>
                        </button>
                      )}
                      <button
                        ...
                        <div className="flex items-center gap-4 mb-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-400">
                              {selectedModel === 'model_1' 
                                ? `UNet++ B8 ${useTTA ? '+ TTA (6x)' : ''}+ Análisis Morfológico`
                                : 'UNet++ ConvNeXt-Base + CBAM (Sin TTA)'
                              }
                            </p>
                      ...
                      <div className="rounded-2xl overflow-hidden border border-slate-700">
                        ...
                        <div className="bg-slate-800 p-3 text-center border-t border-slate-700">
                          <p className="text-sm text-slate-400 font-medium">
                            Resultado Procesado • {result.procesamiento?.model === 'model_1' ? 'Modelo 1' : 'Modelo 2'}
                            {result.procesamiento?.tta_usado && <span className="text-cyan-400"> • TTA {result.procesamiento.tta_transforms}x</span>}
                            {result.procesamiento?.cbam_enabled && <span className="text-purple-400"> • CBAM</span>}
                          </p>
                        </div>
                      </div>
                    )}
            ...
            <div className="space-y-6">
              <div className="relative bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-3xl p-6 md: p-8">
                <h3 className="text-2xl font-bold text-white mb-6 flex items-center gap-3">
                  ...
                  {result ?  'Resultados del Análisis' : 'Tecnología IA v6.0 🚀'}
                </h3>

                {! result ? (
                  <div className="space-y-6">
                    <div className="bg-slate-900/50 border border-slate-700 rounded-2xl p-6">
                      <h4 className="font-bold text-cyan-400 mb-4 text-lg">🧠 Arquitecturas Disponibles</h4>
                      <ul className="space-y-3 text-slate-300 text-sm">
                        <li className="flex items-start gap-2">
                          <CheckCircle className="w-5 h-5 text-cyan-400 flex-shrink-0 mt-0.5" />
                          <span><strong>Modelo 1:</strong> UNet++ + EfficientNet-B8 + TTA (6x)</span>
                        </li>
                        <li className="flex items-start gap-2">
                          <CheckCircle className="w-5 h-5 text-purple-400 flex-shrink-0 mt-0.5" />
                          <span><strong>Modelo 2:</strong> UNet++ + ConvNeXt-Base + CBAM Attention</span>
                        </li>
                        <li className="flex items-start gap-2">
                          <CheckCircle className="w-5 h-5 text-cyan-400 flex-shrink-0 mt-0.5" />
                        ...
                        <li className="flex items-start gap-2">
                          <CheckCircle className="w-5 h-5 text-cyan-400 flex-shrink-0 mt-0.5" />
                          <span><strong>Selección dinámica</strong> de modelo por request</span>
                        </li>
                      </ul>
                    </div>
                    ...
                    <div className="bg-green-500/10 border border-green-500/30 rounded-2xl p-6">
                      <h4 className="font-bold text-green-400 mb-3 text-lg">🚀 Backend v6.0 Dual Model</h4>
                      <ul className="space-y-2 text-slate-300 text-sm">
                        <li>• ✅ Soporte para 2 modelos IA simultáneos</li>
                        <li>• ✅ Selección dinámica:  <code className="bg-slate-800 px-2 py-1 rounded text-cyan-400">model=model_1|model_2</code></li>
                    ...
                    <div className="bg-gradient-to-br from-blue-500/10 to-purple-500/10 border border-blue-500/30 rounded-2xl p-6">
                      ...
                      <ul className="space-y-2 text-slate-300 text-sm">
                        ...
                        <li>• Comparación de resultados entre modelos IA</li>
                      </ul>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-6">
                      ...
                      <div className="bg-slate-900/50 border border-slate-700 rounded-2xl p-6">
                        ...
                        <div className="space-y-2 text-sm text-slate-400">
                          <p>
                            ...
                            <span className={`font-bold ml-2 ${result.procesamiento.model === 'model_1' ? 'text-cyan-400' :  'text-purple-400'}`}>
                              {result.procesamiento.model === 'model_1' ? 'MODELO 1' : 'MODELO 2'}
                            </span>
                          </p>
                          ...
                          {result.procesamiento.model === 'model_1' && (
                          ...
                          {result.procesamiento.model === 'model_2' && result.procesamiento.cbam_enabled && (
                            <p><span className="text-slate-500">CBAM Attention:</span> <span className="text-purple-400">✓ Activado</span></p>
                          )}
                    ...
                    <div className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 border border-green-500/30 rounded-2xl p-6">
                      ...
                      <p className="text-sm text-slate-300 mb-3">
                        El sistema ha detectado y clasificado las grietas en la estructura usando{' '}
                        <span className={result.procesamiento?.model === 'model_1' ?  'text-cyan-400 font-bold' : 'text-purple-400 font-bold'}>
                          {result.procesamiento?.model === 'model_1' ? 'MODELO 1' : 'MODELO 2'}
                        </span>.
                      ...
                      <p className="text-xs text-green-400 mt-2 flex items-center gap-1">
                        ...
                        Procesado con Backend v6.0 Dual Model + Multipart (3× más rápido)
                      </p>
                    </div>
                  </div>
                )}
